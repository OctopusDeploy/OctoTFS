...
const http: HttpClient = new HttpClient("action-install-octopus-cli", undefined, {
    keepAlive: false,
});
...
const getDownloadUrl = async (versionSpec: string): Promise<Endpoint> => {
    ...
    try {
        version = new OctopusCLIVersionFetcher(versionsResponse.versions).getVersion(versionSpec);
    ...
    if (version === null) {
        setFailed(`✕ The version specified (${version}) is not available to download.`);
...
export async function installOctopusCli(version: string): Promise<string> {
    ...
    const downloadPath: string = await downloadTool(octopusCliDownload.downloadUrl, dest);
    ...
    const cachePath: string = await cacheDir(extPath, "octopus", octopusCliDownload.version, octopusCliDownload.architecture);
    debug(`Cached to ${cachePath}`);

    const exePath: string = join(cachePath, osPlatform === "win32" ? "octopus.exe" : "octopus");
